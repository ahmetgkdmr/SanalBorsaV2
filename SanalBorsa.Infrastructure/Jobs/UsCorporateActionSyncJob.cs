...
public sealed class UsCorporateActionSyncJob
{
            ...
            try
            {
                ...
                if (adj.Suspicious > 0)
                {
                    _jobs.Schedule<AdjustedCloseRetryJob>(
                        j => j.RetryAsync(MarketType.UsStocks, symbol, 1, CancellationToken.None),
                        AnomalyRetryPolicy.Phase1Delay);
                    _logger.LogWarning(
                        "UsCorporateActionSyncJob — {Symbol} taze veri ham fiyatla çelişiyordu, yazılmadı, alım/satım kapatıldı — retry zinciri başlatıldı.",
                        symbol);
                }
    ...
    /// RawPriceActionConsistencyService). Sadece loglar — retry/kapatma tetiklemez, çünkü bu bütün
    /// geçmişi değerlendiren bir denetim, "taze bir bar" değil.
    /// </summary>
    private async Task AuditRawConsistencyAsync(string symbol, CancellationToken ct)
    {
        var stock = await _uow.Stocks.GetBySymbolAsync(symbol, ct, MarketType.UsStocks);
        ...
        if (result.SplitMismatches.Count > 0)
        {
            ...
            _logger.LogWarning(
                "UsCorporateActionSyncJob — {Symbol} ham fiyat/split tutarsızlığı: {Count} split'te beklenen oran gerçekleşmemiş. Örnek: {Date:yyyy-MM-dd} beklenen×{Expected:0.##} gerçek×{Actual:0.##}",
        ...
        if (result.AdjustedContinuityMismatches.Count > 0)
        {
            ...
            _logger.LogWarning(
                "UsCorporateActionSyncJob — {Symbol} düzeltilmiş fiyat split'te pürüzsüz kalmamış (TV'nin kendi düzeltmesi split'i yutmamış olabilir): {Count} split. Örnek: {Date:yyyy-MM-dd} önce={Before} sonra={After} (oran×{Ratio:0.##})",
        ...
        if (result.UnexplainedJumps.Count > 0)
        {
            ...
            _logger.LogWarning(
                "UsCorporateActionSyncJob — {Symbol} ham fiyatta kayıtlı olaysız açıklanamaz sıçrama: {Count} gün. Örnek: {Date:yyyy-MM-dd} {Prev}→{Close} ({Pct:0.0}%)",
